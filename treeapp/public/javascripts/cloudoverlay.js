 ...
 * @param {Map} Map to put overlay on.
 */
...
CloudOverlay.prototype.draw = function() {
  ...
  for (var i = 0; i <= this.markers_.length; i++) { // from 0 to the end of this marker
    if (this.markers_[i]) {
      ...
      cloud.circle.attr({stroke: 'none', fill: 'r(.5,.5)#00fff-#00fff', opacity: 0.0});
    ...
    for (var j = 0; j < numStars; j++) {
      window.setTimeout(function() {
         if (true) {
           ...
           star.attr({stroke: '#000', fill: '90-#f7df1e-#f7df1e'});
